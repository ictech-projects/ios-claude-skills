...
> **ICT iOS Team** — This skill generates preview-only mock Repositories following the pattern defined in `references/fixture-conventions.md` and `references/preview-wiring.md`. Fixture data must be shaped by the real Model types — never invented structure.
...
Follow `references/preview-wiring.md` to reference the mock from the target View's `#Preview` block (or add one if it doesn't have one yet).
...
- `references/fixture-conventions.md` — how to build realistic nested fixture values from real Model types, with a worked example.
- `references/preview-wiring.md` — how `#Preview` blocks should reference the generated mock, and the incremental-patch behavior when a protocol changes.

...
> **ICT iOS Team** — This skill wires new screens into ICT's navigation architecture following the pattern defined in `${CLAUDE_SKILL_DIR}/references/navigation-patterns.md` and `${CLAUDE_SKILL_DIR}/references/wiring-checklist.md`. Do not deviate.
...
Follow `${CLAUDE_SKILL_DIR}/references/navigation-patterns.md` to identify, in this project:
...
Print every touchpoint from `${CLAUDE_SKILL_DIR}/references/wiring-checklist.md`, marked as either wired or "needs manual wiring — see file/line," so the developer can verify nothing was missed before committing.

---

## Core Instructions

- **Never invent a new navigation architecture.** Wire into whatever pattern already exists in this project; if it doesn't match `${CLAUDE_SKILL_DIR}/references/navigation-patterns.md`, stop and ask rather than improvising.
...
- `${CLAUDE_SKILL_DIR}/references/navigation-patterns.md` — how to identify the route-enum + central-switch + navigation-manager pattern in a given project (and how to recognize when it isn't there).
- `${CLAUDE_SKILL_DIR}/references/wiring-checklist.md` — the concrete multi-touchpoint checklist, with a worked example.

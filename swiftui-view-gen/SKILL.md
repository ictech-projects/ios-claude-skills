...
## Creating new SwiftUI View Feature
- Ask the user for a Figma design link or `.fig` file if they haven't provided one.
- Use `mcp__figma__get_design_context` or `mcp__figma__get_node` to parse the layer hierarchy, layout properties, and structural boundaries.
- Identify the design tokens used (e.g., specific color names, corner radius scales, or typography styles).
- Evaluate component complexity. Do not bundle an entire complex screen into a single file.
- Split structural blocks into standalone reusable subviews (e.g., `HeaderView`, `CardRow`, `ActionButton`).
- Map standard buttons, inputs, and toggles directly into native SwiftUI elements or existing custom project components.
- **Layouts**: Map Figma Auto-Layout directly to `VStack`, `HStack`, or `ZStack` structures, translating padding properties to `.padding()` modifiers.
- **Colors & Typography**: Map Figma variable tokens directly to your localized semantic asset catalog or font extensions (e.g., `Color.themePrimary`, `Font.customTitle`).
- **Icons**: Automatically match standard UI glyphs to Apple's native `Image(systemName:)` SF Symbols whenever possible.
- Generate clean, compilable, production-ready Swift code using standard modern SwiftUI paradigms.
- Every generated file must include a fully structured `#Preview` block.
- Define clear, strongly-typed data structures for views instead of hardcoding text arrays.
- Ensure accessibility markers are used if design text indicates structural headings (`.accessibilityElement(children: .combine)`).
- All of the UI generation must follow all the rules in `references/views.md` and `references/state-management.md`, including proper state ownership, `@Observable` usage, and avoiding unnecessary view invalidation.

Important steps:
1. Ask the developer for the target file destination path (e.g., `Views/MainScreen.swift`).
2. Write the main view and subviews into the file system.
3. Inform the developer of the mapped tokens and any custom assets they need to add to Xcode manually.

...
> **ICT iOS Team** — This skill cuts release branches and (optionally) exports debug IPAs following ICT's release process. All output must adhere exactly to the patterns defined in `${CLAUDE_SKILL_DIR}/references/branch-and-version.md` and `${CLAUDE_SKILL_DIR}/references/ipa-export.md`. Do not deviate.
...
Follow `${CLAUDE_SKILL_DIR}/references/branch-and-version.md`:
...
Follow the "Version verification" section of `${CLAUDE_SKILL_DIR}/references/branch-and-version.md`: locate the main app target's `MARKETING_VERSION` in `project.pbxproj` and compare it to the version from step 1.
...
If yes, follow `${CLAUDE_SKILL_DIR}/references/ipa-export.md`:
...
- `${CLAUDE_SKILL_DIR}/references/branch-and-version.md` — branch naming (`release/vX.Y.Z`), pre-flight checks, and the version-verification procedure.

...
Scan the chosen scope using the rules in `${CLAUDE_SKILL_DIR}/references/detection-rules.md`. In summary, flag:
...
Follow `${CLAUDE_SKILL_DIR}/references/token-mapping-strategy.md` to propose a replacement for each hit:
...
- `${CLAUDE_SKILL_DIR}/references/detection-rules.md` — literal patterns to flag, with real code examples of what counts as a violation vs. correct token usage.
- `${CLAUDE_SKILL_DIR}/references/token-mapping-strategy.md` — how to map a raw literal to the nearest token, and when to mark a finding unmatched instead.

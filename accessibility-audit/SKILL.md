...
> **ICT iOS Team** — This skill reviews code for accessibility gaps and, with approval, mechanically applies fixes following `references/audit-rules.md` and `references/identifier-convention.md`. It never invents placeholder copy — every label comes from real, adjacent text already in the View.
...
Scan the chosen scope using `references/audit-rules.md`. In summary, flag:
...
- **Traits/hidden/differentiate-without-color** — apply the specific modifier `references/audit-rules.md` calls for; don't over-apply beyond what the finding actually needs.
- **Identifiers** — follow `references/identifier-convention.md` to detect an existing naming convention in the project first; only propose a new convention if none exists, and confirm it with the developer before applying it broadly.
...
- `references/audit-rules.md` — the full check list (VoiceOver, images, Dynamic Type, color differentiation, identifier coverage), building on `swiftui-reviewer`'s existing accessibility checklist.
- `references/identifier-convention.md` — how to detect an existing `accessibilityIdentifier` naming convention in a project, or propose one when none exists.
